...
def getBiasedDataset(data: pd.DataFrame, biases: dict) -> pd.DataFrame:
    ...
    for key in biases:
        ...
        data_s = data[data[key].isin(value_list)]

        df_data = pd.DataFrame(data_s.groupby(by=f'{key}').size(), columns=['Vol']).reset_index().set_index(key)
        ...
        for v in dict_n:
            namespace[f'ds_{count}'] = data_s[data_s[key].isin([v])].sample(n=dict_n[v])
